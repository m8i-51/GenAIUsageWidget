...
**併用テーマ:** A — [GitHub Copilot プロバイダ追加](2026-07-08-github-copilot-provider-design.md)  
**ステータス:** 承認待ち
...
| Copilot | `primary.percent`（なければ `secondary.percent`） | Premium 枠優先。Chat のみプランでは secondary |
...
// providerId: 'claude' | 'codex' | 'cursor' | 'antigravity' | 'copilot'
